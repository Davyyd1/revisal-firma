...
<body>
    ...
    <body>
        ...
        <p style="font-size:14.5px;">Subsemnatul/a <b>{{ autocomplete($employee_co->nume) }} {{ autocomplete($employee_co->prenume) }}</b>, angajat/a in cadrul
            <b>{{ autocomplete($employee_co->companie) }}</b>, in functia de <b>{{ autocomplete($employee_co->functie) }}</b>, avand contract individual de munca pe perioada determinata/nedeterminata,
            va rog sa imi aprobati <b>{{ autocomplete($employee_co->nr_zile) }} zile</b> pentru: </p>
        ...
        @elseif(!$employee_co->data_cerere)
            <p style="font-size:14.5px; margin-left:35rem;">
                Data solicitare cerere: <b>{{ autocomplete($employee_co->data_cerere) }}</b>
                Semnatura salariat:
            </p>
        @endif
